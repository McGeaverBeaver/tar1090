#!/usr/bin/env python3
"""tar1090_heatmap_import: reconstruct the searchable flight index from readsb's heatmap
.ttf chunks. Shared by backfill_heatmap.py (CLI) and tar1090-history-api.py (the Settings
"Historical import" job, which adds DB-backed progress, checkpointing and resume).
...
-- 30-minute binary chunks of every position it saw. This walks those chunks, reconstructs
each aircraft's legs by time gaps, and populates the aircraft + flights tables the history
search uses. Trails themselves are still read on demand from these same .ttf files -- only
the lightweight index is written here.

LIMITATION -- no callsign: heatmap chunks store only hex + position + altitude (+groundspeed
and a source flag), NOT callsign/squawk. Imported flights have callsign = NULL; you search
them by registration / type / operator / military / time. Those come from the aircraft table
-- pass an AIRCRAFT_CSV (wiedehopf aircraft.csv.gz) to fill them in for aircraft the live
logger never saw. For callsigns on old data, use trace_full_*.json + backfill_globe_history.

Heatmap record format (mirrors html/script.js and tar1090-history-api.py): little-endian
int32 quads [hex(+flags), lat*1e6, lon*1e6, gs<<16|alt]; each ~15 s slice is preceded by a
HEAT_MAGIC marker. Bit 24 of the hex word marks a non-ICAO (TIS-B/ADS-R "~") address --
skipped so the registry index stays to real ICAO aircraft.

Config via env: GAP_MINUTES (leg split, default 15), MIN_POINTS (min fixes/leg, default 2).
"""
...
log = logging.getLogger("heatmap-import")
...
# shadow-duplicating the callsign-bearing flights the logger / trace backfill recorded, and
# makes re-running (and resume-after-crash) idempotent alongside the UNIQUE(hex,start_time).
FLIGHT_INSERT = """
...
# --- importer ---------------------------------------------------------------
class Importer:
    """Stream heatmap chunks oldest->newest, segmenting each hex's fixes into flight legs.

    Hooks let the API run it as a resumable background job:
      resume_from   epoch; chunks before it are skipped (the saved checkpoint)
      cancel()      -> True to stop gracefully (state stays resumable)
      on_progress(stats)  called after each chunk and at the end; stats has chunks_total,
                          chunks_done, flights, fixes, checkpoint (= safe resume epoch)
    """

    def __init__(self, conn, meta, *, resume_from=0.0, cancel=None, on_progress=None):
        self.conn = conn
        self.meta = meta
        self.resume_from = resume_from or 0.0
        ...
        self.skipped = 0
        self.processed = 0
        self.cur_cbase = self.resume_from
        self.committed_checkpoint = self.resume_from
    ...
    def emit_leg(self, hexid, leg):
        ...
        sdt = datetime.fromtimestamp(start, tz=timezone.utc)
        edt = datetime.fromtimestamp(last, tz=timezone.utc)
        self.flight_buf.append((hexid, sdt, edt, count, max_alt, hexid, edt, sdt))
        if len(self.flight_buf) >= BATCH:
            self.write_batch()

    def write_batch(self):
        if self.flush_aircraft or self.flight_buf:
            ...
            if not getattr(self.conn, "autocommit", False):
                self.conn.commit()
        # everything emitted so far is now durable; the safe resume point is the start of the
        # oldest still-open leg (or, if none, where we are) -- never ahead of committed data.
        self.committed_checkpoint = self.current_checkpoint()

    def current_checkpoint(self):
        if self.legs:
            return min(self.cur_cbase, min(lg[0] for lg in self.legs.values()))
        return self.cur_cbase
    ...
    def stats(self):
        return {"chunks_total": self.total, "chunks_done": self.skipped + self.processed,
                "flights": self.n_flights, "fixes": self.n_points,
                "checkpoint": self.committed_checkpoint}

    def run(self, root):
        chunks = chunk_files(root)
        self.total = len(chunks)
        if not chunks:
            log.error("no heatmap chunks under %s (expected <Y>/<M>/<D>/heatmap/NN.bin.ttf)", root)
            self.on_progress(self.stats())
            return "done"
        log.info("scanning %d heatmap chunks under %s (resume_from=%s)",
                 self.total, root, int(self.resume_from))
        for cbase, path in chunks:
            if cbase < self.resume_from:
                self.skipped += 1
                continue
            self.cur_cbase = cbase
            self.expire(cbase)
            ...
            if parsed:
                ...
                for si, spos in enumerate(sl):
                    ...
                    while j < n and a[j] != HEAT_MAGIC:
                        w = a[j]
                        if not (w & NONICAO_BIT):
                            self.add(format(w & 0xFFFFFF, "06x"), t, heat_alt(a[j + 3]))
                        j += 4
            self.write_batch()                       # flush + checkpoint every chunk
            self.processed += 1
            self.on_progress(self.stats())
            if self.cancel():
                log.info("import cancelled at checkpoint %s", int(self.committed_checkpoint))
                return "cancelled"
        for h, lg in list(self.legs.items()):        # flush everything still open
            self.emit_leg(h, lg)
        self.legs.clear()
        self.cur_cbase = max((c for c, _ in chunks), default=self.cur_cbase) + CHUNK_SECS
        self.write_batch()
        self.on_progress(self.stats())
        log.info("done: %d flight legs from %d fixes", self.n_flights, self.n_points)
        return "done"
